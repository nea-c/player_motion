...
execute store result score #passenger_effective PlayerMotion.X run attribute @s minecraft:explosion_knockback_resistance get
execute if score #passenger_effective PlayerMotion.X matches 1 run return 0

# Measure a conservative lower bound. Zero (including a -1 total modifier) cannot be boosted.
execute store result score #passenger_scaled PlayerMotion.X run attribute @s minecraft:explosion_knockback_resistance get 1000000000
